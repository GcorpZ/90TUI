...
| `core/theme.rs` | Paleta pastel Clipper | `Clipper{...}` preset + `Turbo` opt | preset |
...
### B) PRIMITIVAS — el pincel de la época
...
* `widgets/list.rs`: lista centrada con selección azul (como el diálogo de índices de referencia). Params: `items, selected, on_key(↑↓Enter)`.
...
* `widgets/table.rs`: `browse(headers, rows, col_widths)` estilo tabla de cargos de la época, con header gris, cursor gris, footer `Linea: 1/0 Total:`.
* `widgets/dialog.rs`: `confirm(), info(), error()` — modal mint o gris según severidad.
* `widgets/progress.rs`: `dual_progress(info: {space, elapsed, file, pct1, cur1/total1, pct2...})` — dos barras gris+azul como captura de referencia.
...
7. Si abre otro modal (diálogo → progreso), se apilan 2 `save()`. Al cerrar, pops en orden. Igual que los 3 niveles de la captura de referencia.
...
Status{left, right} | Confirm{question, [Si,No]} | Launcher{apps:[...]}
```
...
* Empieza por `examples/clipper_shell.rs` (10 líneas) → salta a `widgets/menubar.rs` → baja a `prim/window.rs` → termina en `core/buffer.rs`. De arriba (fácil) a abajo (VRAM).
* Tests en `tests/clipper_golden.rs`: buffers esperados vs reales (snapshot), garantizan que sombra/hotkey no se rompan.
