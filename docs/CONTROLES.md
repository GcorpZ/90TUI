...
| `calendar_draw(buf, rect, cal)` | `CalendarPicker::new/current()` (matemática civil propia) + `is_open` | campo 1 fila ` [ DD/MM/AAAA ] [ 󰃭 ]` (`\u{f073}`); abierto: popup 23×10 (`calendar_popup_rect`, debajo o encima) con caja doble CP437 `╔═╗║╚╝`, cabecera `«◄ MES AAAA ►»`, semana `Lu..Do`, cursor Clipper; `calendar_key` (`Enter`/Espacio abre) y `calendar_key_mod` (flechas días, `PgUp/PgDn` mes, `Shift/Ctrl`+`PgUp/PgDn` año, `Enter` acepta, `Esc` restaura) |
