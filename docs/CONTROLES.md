...
| `icons20::{drive_badge, folder_badge, file_badge, fkey_badge}` (`IconMode::{RetroCp437, NerdFont}`) | `DriveType::{Floppy35, Floppy525, HardDisk, CdRom}`, activo; `bg` del llamante | `u16`=1–7 | carcasas `[≡]/[═]/[▬-]/[○]` (NF solo floppy `U+F0A0`), `[+]/[-]` sin parches, `*`/`≡`/`·` + aire (nombre en x+2), `F1 Help` Norton + 2 aire |
